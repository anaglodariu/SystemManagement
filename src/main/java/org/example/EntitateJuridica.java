...
public class EntitateJuridica extends Utilizator {
    ...
    public String textCerere(Cerere.TipCerere tipCerere) {
        StringBuffer mesaj = new StringBuffer("Utilizatorul de tip entitate juridica nu poate inainta o cerere de tip ");
        try {
            switch (tipCerere) {
                ...
                case INLOCUIRE_BULETIN:
                    mesaj.append("inlocuire buletin");
                    throw new ExceptieSolicitare(mesaj.toString());
                case INLOCUIRE_CARNET_DE_SOFER:
                    mesaj.append("inlocuire carnet de sofer");
                    throw new ExceptieSolicitare(mesaj.toString());
                default:
                    return "Subsemnatul " + reprezentant + ", reprezentant legal al companiei " + nume
                            + ", va rog sa-mi aprobati urmatoarea solicitare: " + tipCerere.toString().replace("_", " ").toLowerCase();
            }
    ...
    public void adaugareCerere(Cerere cerere) {
        cereriInAsteptare.add(cerere); /*adauga cererile in ordinea datelor*/
    }
